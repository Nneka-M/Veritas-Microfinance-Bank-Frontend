...
import { z } from "zod";
import { ArrowLeft, ArrowUpFromLine, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function WithdrawPage() {
    ...
    useEffect(() => {
        accountsApi.getAll()
            .then(res => setAccounts(res.data.filter((a: Account) => a.is_active)))
            .catch(console.error)
            .finally(() => setLoading(false));
    }, []);

    const isFixedLocked = (acc?: Account) => {
        if (!acc || acc.account_type !== "FIXED") return false;
        if (!acc.maturity_date) return false;
    ...
    const onSubmit = async (data: FormData) => {
        setError("");
        if (isFixedLocked(selectedAccount)) {
            setError("This fixed deposit account has not matured yet and cannot be withdrawn from.");
            return;
        }
        try {
            const res = await transactionsApi.withdraw(data);
            setResult(res.data);
        } catch (err: any) {
    ...
    if (result) {
        return (
            <div className="max-w-md mx-auto text-center py-12">
                ...
                <h2 className="text-xl font-semibold text-ink-primary mb-1">Withdrawal successful</h2>
                <p className="text-sm text-ink-secondary mb-6">Funds have been removed from your account</p>

                <Card className="text-left mb-6">
                    {[
                        ["Reference", result.reference],
                        ["Amount withdrawn", `₦${Number(result.amount).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`],
                        ["Remaining balance", `₦${Number(result.new_balance).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`],
    ...
    return (
        <div className="max-w-md">
            ...
            <h1 className="text-2xl font-semibold text-ink-primary mb-1">Withdraw funds</h1>
            <p className="text-sm text-ink-secondary mb-6">Move money out of your account</p>

            {loading ? (
                <div className="flex justify-center py-12"><Spinner size="lg" /></div>
            ) : accounts.length === 0 ? (
                <Card className="text-center py-10">
                    <p className="text-sm text-ink-secondary">No active accounts found.</p>
                </Card>
            ) : (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="space-y-2">
                            {accounts.map((acc) => {
                                ...
                                const locked = isFixedLocked(acc);
                                return (
                                    <div
                                        key={acc.account_id}
                                        onClick={() => !locked && setValue("account_id", acc.account_id)}
                                        className={`flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all duration-150
                      ${locked ? "opacity-50 cursor-not-allowed border-surface-200" :
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {locked && (
                                                    <span className="text-[10px] font-medium text-warning bg-amber-50 px-1.5 py-0.5 rounded-md border border-amber-100">
                                                        LOCKED
                                                    </span>
                                                )}
                                            </div>
                                            <p className="text-xs text-ink-tertiary font-mono">{acc.account_number}</p>
                                            {locked && acc.maturity_date && (
                                                <p className="text-xs text-warning mt-0.5">
                                                    Matures {new Date(acc.maturity_date).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}
                                                </p>
                                            )}
                                        </div>
                  ...
                  ${insufficientFunds ? "border-danger focus:ring-danger" : "border-surface-200"}
                `}
                                {...register("amount")}
                            />
                        </div>
                        ...
                        {selectedAccount && amount > 0 && !insufficientFunds && (
                    ...
                    <Button
                        ...
                        size="lg"
                        className="w-full"
                        disabled={!selectedAccountId || !!insufficientFunds || isFixedLocked(selectedAccount)}
